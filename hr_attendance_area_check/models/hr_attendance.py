...
class HrAttendance(models.Model):
    ...
    def _reverse_geocode(self, lat, lon):
        """เรียก Nominatim — ถ้า error หรือ rate limit → return None (ปล่อยผ่าน)"""
        ...
        try:
            ...
            if resp.status_code != 200:
                _logger.info("reverse_geocode HTTP %s — skipped", resp.status_code)
                return None
            data = resp.json()
            ...
            _logger.warning("reverse_geocode error: %s", e)
        return None
